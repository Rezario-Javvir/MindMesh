...
function CardItem({ article }) {
  ...
  const detailLink = `/article/${article.id}`
    ...
    <Link to={detailLink} className='block'> 
      ...
      >
        ...
        <div className='relative w-full p-3 neuro-in rounded-md text-gray-800 bg-white bg-opacity-90'>
          ...
          <h3 className='font-bold text-lg md:text-xl line-clamp-2'>{article.title}</h3>
          
          <p className='text-sm mt-1 line-clamp-1'>{article.content}</p>
